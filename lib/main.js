...
module.exports.debug = null

// Private
function packagesToInstall(packageName, enablePackages) {
  ...
  if (exports.debug) {
    exports.debug.emit('packagesToInstall', packagesToInstall)
  }
...
export function install(packageName, enablePackages = false) {
  ...
  const toInstall = packagesToInstall(packageName, enablePackages)
  if (toInstall.length) {
    return installPackage(packageName, toInstall)
...
function installPackage(packageName, packageNames) {
  ...
  return view.createNotification().then(() =>
    installPackages(packageNames, function(name) {
      if (exports.debug) {
        exports.debug.emit('installPackage', name)
      }
      view.markFinished()
      atom.packages.enablePackage(name)
      atom.packages.activatePackage(name)
